...
export class KitchenController {
  ...
  @ApiOperation({ summary: 'Verify if selected batch is the oldest available (FEFO compliance)' })
  checkFefo(@Query('productId', ParseIntPipe) productId: number, @Query('batchNumber') batchNumber: string) {
  ...
  getCostPerMealHistory(@Query('days') days?: string) {
    return this.kitchenService.getCostPerMealHistory(days ? parseInt(days, 10) : 7);
  }

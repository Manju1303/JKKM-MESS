...
export class ReportsService {
  ...
  async getReportFile(id: number) {
    ...
    if (report.type === "DAILY") {
      const datePart =
        ...
        new Date(report.dateFrom).toISOString().split("T")[0];
      const { buffer, filename } = await this.generateDailyReport(
        datePart,
        report.generatedBy,
      );
      ...
      const { buffer, filename } = await this.generateMonthlyReport(
        year,
        month,
        report.generatedBy,
      );
      ...
      const { buffer, filename } = await this.generateInventoryReport(
        report.generatedBy,
      );
      return { buffer, filename };
    }
  ...
  private applyTotalRowStyling(
    sheet: ExcelJS.Worksheet,
    netAmountColNum: number,
  ) {
  ...
  private applyPremiumStyling(sheet: ExcelJS.Worksheet) {
    ...
    sheet.eachRow((row, rowNumber) => {
      ...
      if (
        ...
        row.getCell(1).value?.toString().includes("TOTAL")
      )
        return;
      row.height = 20;

  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleConnection(client: Socket) {
    try {
      ...
      if (!authHeader) {
        this.logger.warn(
          `Disconnecting unauthenticated WS client: ${client.id}`,
        );
        client.disconnect();
        return;
      }
      ...
      this.logger.log(
        `Client connected: ${client.id} (User: ${payload.email}) | Total: ${this.connectedClients.size}`,
      );
      client.emit("connected", {
        message: "Connected to JKKM Mess ERP",
        clientId: client.id,
      });
    } catch (err) {
      this.logger.warn(
        `Invalid JWT on WS connect, disconnecting ${client.id}: ${err.message}`,
      );
      client.disconnect();
    }
  }
  ...
  handleJoinRoom(client: Socket, room: string) {
    ...
    if (!user) {
      this.logger.warn(
        `Unauthenticated client ${client.id} tried to join room ${room}`,
      );
      client.disconnect();
    ...
    if (
      room === "managers" &&
      !["SUPER_ADMIN", "MESS_MANAGER"].includes(user.role)
    ) {
      this.logger.warn(
        `User ${user.email} with role ${user.role} unauthorized to join room ${room}`,
      );
      return { event: "error", message: "Unauthorized" };
    }
    client.join(room);
    this.logger.log(
      `Client ${client.id} (User: ${user.email}, Role: ${user.role}) joined room: ${room}`,
    );

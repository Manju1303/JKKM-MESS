...
export class AuthService {
  ...
  private validateDomain(email: string) {
    ...
    if (!cleaned.endsWith("@jkkm.edu.in")) {
      throw new ForbiddenException(
        "Access restricted to JKKM institutional accounts only.",
      );
    }
  }
  ...
  async validateUser(
    ...
    if (!isMatch) {
      ...
      if (isLocking) {
        throw new UnauthorizedException(
          "Account is locked due to 5 failed attempts. Locked for 15 minutes.",
        );
      } else {
        throw new UnauthorizedException(
          `Invalid credentials. Attempts remaining: ${5 - attempts}`,
        );
      }
    }
